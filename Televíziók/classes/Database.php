...
class Database {
    ...
    public function setTv($tvid, $tv_neve, $ar, $marka, $kepatlo, $hz,$felbontas, $kijelzo, $megjegyzes) {
        $stmt = $this->db->prepare("UPDATE `tv` SET `tv_neve`= ?,`ar`= ?,`marka`= ?,`kepatlo`= ?,`hz`= ?,`felbontas`= ?,`kijelzo`= ?,`megjegyzes`= ? WHERE tvid= ?");
        $stmt->bind_param('issssssss', $tvid, $tv_neve, $ar, $marka, $kepatlo, $hz,$felbontas, $kijelzo, $megjegyzes);
        return $stmt->execute();
    }

    public function getTvek() {
        $result = $this->db->query("SELECT DISTINCT `marka` FROM `tv`;");
        return $result->fetch_all();
    }
    
    public function setTvvasarlas($tvid, $userid) {
